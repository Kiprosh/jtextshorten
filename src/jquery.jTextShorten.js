(function( $ ){
  ...
  methods = {
    init: function( options ){
      return this.each(function()
      {
        var tag,
            $this = $( this ),
            uniq_stamp = generateUniqStamp(),
            settings = $.extend( {}, _defaults ),
            excluded_tags = [ "INPUT", "IMG", "SELECT", "IFRAME" ];
        ...
        if( $.inArray( this.tagName, excluded_tags ) == -1 ) {
          ...
          throw new Error( "jQuery.shortened cannot be applied on " + tag +
            " tag." );
        }
      });
    },
  ...
  function collapse( element ){
    ...
    $( ".ui-see_less[data-uniq=" + options.unique_identity + "]" ).
      removeClass( 'ui-see_less' ).addClass( 'ui-see_more' ).
  ...
  function expand( element ){
    ...
    $( ".ui-see_more[data-uniq=" + options.unique_identity + "]" ).
      removeClass( 'ui-see_more' ).addClass( 'ui-see_less' ).
  ...
  function determineTextLength( element, stamp, settings ){
    if ( element.text().length > settings.limitChars ) {
      ...
      element.html( text );
      seeMoreTag = "<span class='ui-see_more ui-blue' data-uniq=" + stamp +
        ">See more</span>";
      $( seeMoreTag ).insertAfter( element );
    }
  }
  ...
  $.fn.shortened = function( method )
  {
    if( methods[ method ] ){
      ...
      $.error( 'Method ' + method + ' does not exist on jQuery.shortened' );
    }
  }
}(jQuery));
